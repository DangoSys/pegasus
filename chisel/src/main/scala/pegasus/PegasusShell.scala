...
class PegasusShell extends BlackBox with HasBlackBoxInline {
  ...
  setInline("PegasusShell.v",
    """module PegasusShell (
      ...
      |  );
      |
      |  // ── AXI Interconnect 2→1 (ui_clk domain) ─────────────────────────
      |  // axi_interconnect v1.7 standalone uses concatenated vector ports (not S00/S01 named).
      |  // [1] = S01(SoC), [0] = S00(H2C) — LSB is first slave
      |  axi_ic_ddr4 axi_ic (
      |    .aclk(ddr4_ui_clk), .aresetn(ddr4_aresetn),
      |    .s_axi_awid   ({cs_bid,     4'h0}),
